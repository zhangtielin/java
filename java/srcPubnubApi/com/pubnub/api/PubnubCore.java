...
abstract class PubnubCore {
	...
	private String getState() {
		return (subscriptions.state.length() > 0) ? subscriptions.state
				.toString() : null;
	}
	...
	class PresenceHeartbeatTask extends TimedTask {
		...
		public void run() {
			...
			HttpRequest hreq = new HttpRequest(urlComponents, parameters,
					new ResponseHandler() {
						public void handleResponse(HttpRequest hreq,
								String response) {
							JSONObject jso;
							try {
								...
								handleError(hreq, PubnubError.getErrorObject(
										PubnubError.PNERROBJ_INVALID_JSON, 1,
										response));
								return;
							}
							callback.successCallback(
									subscriptions.getChannelStringNoPresence(),
									response);
						}

						public void handleError(HttpRequest hreq,
								PubnubError error) {
	...
	public void setHeartbeat(int heartbeat, Callback callback) {
		...
		if (PRESENCE_HEARTBEAT_TASK == 0) {
			PRESENCE_HEARTBEAT_TASK = timedTaskManager.addTask(
					"Presence-Heartbeat", new PresenceHeartbeatTask(
			...
			timedTaskManager.updateTask(PRESENCE_HEARTBEAT_TASK,
	...
	protected String getPubnubUrl() {
		...
		if (ORIGIN_STR == null) {
			...
			ORIGIN_STR += HOSTNAME;
			ORIGIN_STR += ((!this.CACHE_BUSTING) ? "" : "-"
					+ String.valueOf(HOSTNAME_SUFFIX));
			ORIGIN_STR += "." + DOMAIN;
		}
		return ORIGIN_STR;
	}
	...
	private boolean validateInput(String name, Object input, Callback callback) {
		...
		if (input == null) {
			callback.errorCallback("", PubnubError.getErrorObject(
					PubnubError.PNERROBJ_INVALID_ARGUMENTS, 1, name
							+ " cannot be null"));
			return false;
		}
		...
		if (input instanceof String && ((String) input).length() == 0) {
			callback.errorCallback("", PubnubError.getErrorObject(
					PubnubError.PNERROBJ_INVALID_ARGUMENTS, 2, name
	...
	public void publish(String channel, JSONObject message,
			boolean storeInHistory, Callback callback) {
	 ...
	 *            object of sub class of Callback class
	 */
	public void publish(String channel, JSONArray message,
			boolean storeInHistory, Callback callback) {
		Hashtable args = new Hashtable();
		args.put("channel", channel);
		args.put("message", message);
	 ...
	 *            object of sub class of Callback class
	 */
	public void publish(String channel, String message, boolean storeInHistory,
			Callback callback) {
		Hashtable args = new Hashtable();
		args.put("channel", channel);
		args.put("message", message);
	 ...
	 *            object of sub class of Callback class
	 */
	public void publish(String channel, Integer message,
			boolean storeInHistory, Callback callback) {
		Hashtable args = new Hashtable();
		args.put("channel", channel);
		args.put("message", message);
	 ...
	 *            object of sub class of Callback class
	 */
	public void publish(String channel, Double message, boolean storeInHistory,
			Callback callback) {
		Hashtable args = new Hashtable();
		args.put("channel", channel);
		args.put("message", message);
	...
	protected void publish(Hashtable args) {
		...
		final Callback callback = getWrappedCallback((Callback) args
				.get("callback"));
		...
		if (this.CIPHER_KEY.length() > 0) {
			...
			try {
				...
				callback.errorCallback(channel, PubnubError.getErrorObject(
						PubnubError.PNERROBJ_ENCRYPTION_ERROR, 1, msgStr));
				return;
			} catch (IllegalStateException e) {
				callback.errorCallback(channel, PubnubError.getErrorObject(
						PubnubError.PNERROBJ_ENCRYPTION_ERROR, 2, msgStr));
				return;
			} catch (InvalidCipherTextException e) {
				callback.errorCallback(channel, PubnubError.getErrorObject(
						PubnubError.PNERROBJ_ENCRYPTION_ERROR, 3, msgStr));
				return;
			} catch (Exception e) {
				callback.errorCallback(channel, PubnubError.getErrorObject(
						PubnubError.PNERROBJ_ENCRYPTION_ERROR, 4, msgStr
								+ " : " + e.toString()));
				return;
			}
		} else {
	...
	public void setState(String channel, String uuid, JSONObject state,
			Callback callback) {
	...
	protected void invokeCallback(String channel, String response, String key,
		...
		invokeCallback(channel, response, key, callback, extendedErrorCode,
				false);
	}
	...
	protected void invokeCallback(String channel, String response, String key,
		...
		try {
			...
			callback.errorCallback(channel, PubnubError.getErrorObject(
					PubnubError.PNERROBJ_JSON_ERROR, extendedErrorCode,
					response));
			return;
		}
	 ...
	 *            object of sub class of Callback class
	 */
	public void hereNow(final String channel, boolean state, boolean uuids,
			Callback callback) {
		...
		if (channel != null && channel.length() > 0) {
			urlargs = new String[] { getPubnubUrl(), "v2", "presence",
					"sub_key", this.SUBSCRIBE_KEY, "channel",
					PubnubUtil.urlEncode(channel) };
		} else {
			urlargs = new String[] { getPubnubUrl(), "v2", "presence",
					"sub_key", this.SUBSCRIBE_KEY };
		}
	 ...
	 */
	public void history(final String channel, long start, long end, int count,
			boolean reverse, boolean includeTimetoken, Callback callback) {
		final Callback cb = getWrappedCallback(callback);
		Hashtable parameters = PubnubUtil.hashtableClone(params);
		if (count == -1)
			count = 100;
		...
		class HistoryResponseHandler extends ResponseHandler {
			...
			public void handleResponse(HttpRequest hreq, String response) {
				...
				try {
					...
					cb.errorCallback(channel, PubnubError.getErrorObject(
							PubnubError.PNERROBJ_JSON_ERROR, 3));
				} catch (DataLengthException e) {
					cb.errorCallback(channel, PubnubError.getErrorObject(
							PubnubError.PNERROBJ_DECRYPTION_ERROR, 6, response));
				} catch (IllegalStateException e) {
					cb.errorCallback(channel, PubnubError.getErrorObject(
							PubnubError.PNERROBJ_DECRYPTION_ERROR, 7, response));
				} catch (InvalidCipherTextException e) {
					cb.errorCallback(channel, PubnubError.getErrorObject(
							PubnubError.PNERROBJ_DECRYPTION_ERROR, 8, response));
				} catch (IOException e) {
					cb.errorCallback(channel, PubnubError.getErrorObject(
							PubnubError.PNERROBJ_DECRYPTION_ERROR, 9, response));
				} catch (Exception e) {
					cb.errorCallback(channel, PubnubError.getErrorObject(
							PubnubError.PNERROBJ_DECRYPTION_ERROR, 10, response
									+ " : " + e.toString()));
				}
	 ...
	 *            Callback object
	 */
	public void history(String channel, boolean includeTimetoken, int count,
			Callback callback) {
	 ...
	 *            Callback
	 */
	public void history(String channel, long start, int count, boolean reverse,
			Callback callback) {
	...
	private boolean isWorkerDead(HttpRequest hreq) {
		return (hreq == null || hreq.getWorker() == null) ? false : hreq
				.getWorker()._die;
	}
		...
		HttpRequest hreq = new HttpRequest(urlComponents, params,
				new ResponseHandler() {
												...
												if (!isWorkerDead(hreq))
													_channel.callback
															.errorCallback(
																	...
																	PubnubError
																			.getErrorObject(
																					PubnubError.PNERROBJ_DECRYPTION_ERROR,
																					11,
																					messages.get(
																							i)
																							.toString()));
					...
					public void handleTimeout(HttpRequest hreq) {
						...
						subscriptions.invokeErrorCallbackOnChannels(PubnubError
								.getErrorObject(PubnubError.PNERROBJ_TIMEOUT, 1));
						// disconnectAndResubscribe();
	...
	void get(String objectId, String path, String nextPage, Callback callback) {
		...
		if (path != null && path.length() > 0) {
			urlargs = new String[] { getPubnubUrl(), "v1", "datasync",
	...
	private void write(String objectId, String path, JSONObject jso,
		...
		if (path != null && path.length() > 0) {
			urlargs = new String[] { getPubnubUrl(), "v1", "datasync",
					"sub-key", this.SUBSCRIBE_KEY, "pub-key", this.PUBLISH_KEY,
					"obj-id", PubnubUtil.urlEncode(objectId),
					PubnubUtil.urlEncode(path) };
		} else {
			urlargs = new String[] { getPubnubUrl(), "v1", "datasync",
					"sub-key", this.SUBSCRIBE_KEY, "pub-key", this.PUBLISH_KEY,
					"obj-id", PubnubUtil.urlEncode(objectId) };
